...
from stage2_dataset import MSD_Liver_LiQA_Dataset
...
def test(rank, world_size, args, shared_list):
    ...
    val_dataset = MSD_Liver_LiQA_Dataset(modality=args.modality, mode='inference')
    ...
    for i, data in enumerate(progress_bar):
        ...
        for j in range(0, len(generators), args.num_images_per_prompt):
            ...
            for k, output_image in enumerate(sequence):
                ...
                # item.update({f"mask_{m}_path": os.path.join(logdirs['mask'], f"{absolute_index}_mask_{m}.png") for m in range(2)})
...
def main():
    ...
    parser.add_argument('--modality', type=str, choices=['T1', 'CT'], required=True)
