...
from stage2_dataset import LiQA_CHAOS_MRI_Dataset
from utils.util import calculate_iou, get_bbox_from_mask, save_images
...
def test(rank, world_size, args, shared_list):
    ...
    predictor = SAM2ImagePredictor(build_sam2(model_cfg, checkpoint)) # load SAM2 model for post-filtering
    ...
    val_dataset = LiQA_CHAOS_MRI_Dataset(modality=args.modality, mode='inference')
    ...
    for i, data in enumerate(progress_bar):
        ...
        # already set organs to Liver
        if args.modality == 'T2-SPIR':
            prompt = [", ".join(['T2 SPIR Abdomen MRI, fat low signal, muscle low signal, water high signal, fat dark, water bright', data['region'][0], data['organs'][0]])]    
        elif args.modality == 'T1':
            prompt = [", ".join(['T1 Abdomen MRI, fat high signal, muscle intermediate signal, water low signal, fat bright, water dark', data['region'][0], data['organs'][0]])]
                
        # if args.modality == 'T2':
        #     prompt = [", ".join(['T2 Abdomen MRI, fat moderate signal, muscle low signal, water high signal, fat moderately bright, muscle dark', data['region'][0], data['label'][0]])]    
        # elif args.modality == 'ADC-PROMISE' or args.modality == 'ADC-MSD':
        #     prompt = [", ".join(['ADC Abdomen MRI, fat low signal, muscle low signal, water high signal, water bright', data['region'][0], data['label'][0]])]
        ...
        mask_images = [mask[:, [j], :, :] for j in range(1)]
        mask_file_names = [f"{absolute_index}_mask_{j}.png" for j in range(1)]
            ...
            for k, output_image in enumerate(sequence):
                ...
                for m in range(len(mask_images)):
                    ...
                    with torch.inference_mode(), torch.autocast("cuda", dtype=torch.bfloat16):
                        ...
                        masks, scores, logits = predictor.predict(mask_input=resized_mask[None, :, :]) # 传入初始的二值 mask
                ...
                item.update({f"mask_{m}_path": os.path.join(logdirs['mask'], f"{absolute_index}_mask_{m}.png") for m in range(1)})
...
def main():
    ...
    parser.add_argument('--modality', type=str, choices=['T2-SPIR', 'T1'], required=True)
