...
def test(rank, world_size, args, shared_list):
    ...
    val_dataset = Prostate_MRI_Dataset(modality=args.modality, mode='inference')
    ...
    for i, data in enumerate(progress_bar):
        ...
        # if args.modality == 'T2':
        #     prompt = [", ".join(['T2 Abdomen MRI, fat moderate signal, muscle low signal, water high signal, fat moderately bright, muscle dark', data['region'][0], data['organs'][0]])]    
        # elif args.modality == 'ADC-PROMISE' or args.modality == 'ADC-MSD':
        #     prompt = [", ".join(['ADC Abdomen MRI, fat low signal, muscle low signal, water high signal, water bright', data['region'][0], data['organs'][0]])]
        ...
        for j in range(0, len(generators), args.num_images_per_prompt):
            ...
            for k, output_image in enumerate(sequence):
                ...
                item = {
                    ...
                    'prompt': prompt,
                    'original_path': data['image_path'][0],
...
def main():
    ...
    parser.add_argument('--modality', type=str, choices=['T2', 'ADC-PROMISE', 'ADC-MSD'], required=True)
