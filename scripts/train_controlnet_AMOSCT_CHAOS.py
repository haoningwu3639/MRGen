...
class SampleLogger:
    ...
    def log_sample_images(
        ...
        for idx, prompt in enumerate(tqdm(self.prompts, desc="Generating sample images")):
            ...
            sequence = pipeline(
                # prompt=[", ".join(['CT, fat low signal, muscle moderate signal, water low signal, fat dark, muscle gray, water dark', batch['region'][0], batch['organs'][0]])],
                prompt=[", ".join(['CT, fat low signal, muscle moderate signal, water low signal, fat dark, muscle gray, water dark', batch['region'][0], batch['label'][0]])],
                # image=mask,
                image=mask,
            ...
            for i, img in enumerate(sequence):
                img.save(os.path.join(self.logdir, f"{step}_{idx}_{sample_seeds[i]}_output_T1CE.png"))
...
def train(
    ...
    train_dataset = AMOSCT_CHAOS_Dataset(mode='train-T1', target_modality='T1')
    val_dataset = AMOSCT_CHAOS_Dataset(mode='test-T1', target_modality='T1')
    # train_dataset = AMOSCT_CHAOS_Dataset(mode='train-T2', target_modality='T2-SPIR')
    # val_dataset = AMOSCT_CHAOS_Dataset(mode='test-T2', target_modality='T2-SPIR')
    # train_dataset = AMOSCT_CHAOS_Dataset(mode='train-T1', target_modality='CT')
    # val_dataset = AMOSCT_CHAOS_Dataset(mode='test-T1', target_modality='CT')
    # train_dataset = AMOSCT_CHAOS_Dataset(mode='train-T2', target_modality='CT')
    # val_dataset = AMOSCT_CHAOS_Dataset(mode='test-T2', target_modality='CT')
