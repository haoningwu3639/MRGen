...
from stage2_dataset import PanSeg_MRI_Dataset
...
class SampleLogger:
    ...
    def log_sample_images(
        ...
        for idx, prompt in enumerate(tqdm(self.prompts, desc="Generating sample images")):
            ...
            sequence = pipeline(
                # prompt=[", ".join(['T1 Abdomen MRI, fat high signal, muscle intermediate signal, water low signal, fat bright, water dark', batch['region'][0], batch['organs'][0]])],
                prompt=[", ".join(['T1 Abdomen MRI, fat high signal, muscle intermediate signal, water low signal, fat bright, water dark', batch['region'][0], batch['label'][0]])],
                image=mask,
            ...
            for i, img in enumerate(sequence):
                img.save(os.path.join(self.logdir, f"{step}_{idx}_{sample_seeds[i]}_output_T1.png"))
...
def train(
    ...
    # train_dataset = PanSeg_MRI_Dataset(mode='train', modality='T1')
    # val_dataset = PanSeg_MRI_Dataset(mode='test', modality='T1')
    train_dataset = PanSeg_MRI_Dataset(mode='train', modality='T2')
    val_dataset = PanSeg_MRI_Dataset(mode='test', modality='T2')
