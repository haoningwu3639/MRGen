...
from stage2_dataset import Prostate_MRI_Dataset
...
class SampleLogger:
    ...
    def log_sample_images(
        self, batch, pipeline: StableDiffusionControlNetPipeline, device: torch.device, step: int
    ):
        ...
        for idx, prompt in enumerate(tqdm(self.prompts, desc="Generating sample images")):
            ...
            sequence = pipeline(
                # prompt=[", ".join(['ADC Abdomen MRI, fat low signal, muscle low signal, water high signal, water bright', batch['region'][0], batch['organs'][0]])],
                prompt=[", ".join(['ADC Abdomen MRI, fat low signal, muscle low signal, water high signal, water bright', batch['region'][0], batch['label'][0]])],
                image=mask,
            ...
            sequence_T2 = pipeline(
                # prompt=[", ".join(['T2 Abdomen MRI, fat moderate signal, muscle low signal, water high signal, fat moderately bright, muscle dark', batch['region'][0], batch['organs'][0]])],
                prompt=[", ".join(['T2 Abdomen MRI, fat moderate signal, muscle low signal, water high signal, fat moderately bright, muscle dark', batch['region'][0], batch['label'][0]])],
                image=mask,
                height=image.shape[2],
                width=image.shape[3],
                generator=generator,
            ...
            for i, img in enumerate(sequence):
                img.save(os.path.join(self.logdir, f"{step}_{idx}_{sample_seeds[i]}_output_ADC.png"))
            for i, img in enumerate(sequence_T2):
                img.save(os.path.join(self.logdir, f"{step}_{idx}_{sample_seeds[i]}_output_T2.png"))
...
def train(
    ...
    text_encoder = AutoModel.from_pretrained(os.path.join(pretrained_model_path, 'BiomedCLIP'), trust_remote_code=True)
    ...
    train_dataset = Prostate_MRI_Dataset(mode='train', modality='ADC')
    val_dataset = Prostate_MRI_Dataset(mode='test', modality='ADC')
    # train_dataset = Prostate_MRI_Dataset(mode='train', modality='T2')
    # val_dataset = Prostate_MRI_Dataset(mode='test', modality='T2')
