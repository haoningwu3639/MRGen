...
from stage2_dataset import MSD_Liver_CHAOS_MRI_Dataset
...
def test(rank, world_size, args, shared_list):
    ...
    val_dataset = MSD_Liver_CHAOS_MRI_Dataset(modality=args.modality, mode='inference')
    ...
    # 获取已保存列表中的所有samples的index值
    existing_indices = [item['index'] for item in shared_list]
...
def main():
    ...
    parser.add_argument('--modality', type=str, choices=['T2-SPIR', 'CT'], required=True)
